...
void uart_set_baudrate_mine(uint32_t periph_clock, uint32_t baud_rate);
...
// an extra version where we try to set the mantissa and fraction manually
void uart_set_baudrate_mine(uint32_t periph_clock, uint32_t baud_rate){

	float div = periph_clock / baud_rate;

	unsigned long mantissa = (unsigned long) div;
	unsigned long frac = (unsigned long) round((div-mantissa)*16);
