...
void SenderTask1(void *pvParams);
void SenderTask2(void *pvParams);
void ReceiverTask(void *pvParams);

TaskHandle_t SenderHandle1, SenderHandle2, ReceiverHandle;
QueueHandle_t YearQueue;

int32_t send1Profiler, send2Profiler, recvProfiler;
...
void ReceiverTask(void *pvParams){
	...
	while(1){
		...
		qStatus = xQueueReceive(YearQueue, &value_to_receive, _100ms);
		if(qStatus == pdPASS){
			printf("%ld\n\r", value_to_receive);
		}
	}
